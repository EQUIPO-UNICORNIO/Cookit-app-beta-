...
router.get('/', async (req, res) => {
  try {
    const items = await getAll('shopping_items', { user_id: req.userId }, { orderBy: 'created_at' });
...
router.post('/', async (req, res) => {
  try {
    ...
    const item = await create('shopping_items', {
...
router.put('/:id', async (req, res) => {
  try {
    ...
    await updateById('shopping_items', req.params.id, {
      name, category, quantity, unit, checked: checked ? 1 : 0
    }, req.userId);
...
router.delete('/:id', async (req, res) => {
  try {
    await deleteById('shopping_items', req.params.id, req.userId);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
